...
class ESC:
    ...
    def __init__(self, pino_da_rasp,velocidade_atual, ligado=False, calibrada=False):
        self.ligado = ligado
        self.calibrada = calibrada
        self.pino_da_rasp = pino_da_rasp
        print(f'ESC do {self.nome_motor} cadastrada')

    def calibrar(self, pino_da_rasp, calibrada):
    ...
    def definir_velocidade(self, pino_da_rasp,velocidade_atual,velocidade_porcentagem):
        if self.calibrada == True:
            ...
            pi.hardware_PWM(pino_da_rasp, frequencia, duty_cycle)
...
while True:
    velocidade_propulsao = int(input('velocidade desejada para a ESC de propulsão: '))
    velocidade_sustentacao = int(input('velocidade desejada para a ESC de sustentação: '))
